...
import { getPackageKangen } from '../../publics/redux/actions/package/teleponDanSms'

class Kangen extends Component {
    componentDidMount = async () => {
        if (this.props.kangen === undefined) await this.props.dispatch(getPackageKangen())
    }

    render() {
        let { kangen } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(Kangen))
