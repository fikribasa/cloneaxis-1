...
import { getPackageInternetMabrur } from '../../publics/redux/actions/package/roaming'

class InternetMabrur extends Component {
    componentDidMount = async () => {
        if (this.props.internetmabrur === undefined) await this.props.dispatch(getPackageInternetMabrur())
    }

    render() {
        let { internetmabrur } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(InternetMabrur))
