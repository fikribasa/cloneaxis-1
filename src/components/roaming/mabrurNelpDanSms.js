...
import { getPackageMabrurNelpDanSms } from '../../publics/redux/actions/package/roaming'

class MabrurNelpDanSms extends Component {
    componentDidMount = async () => {
        if (this.props.mabrurnelpdansms === undefined) await this.props.dispatch(getPackageMabrurNelpDanSms())
    }

    render() {
        let { mabrurnelpdansms } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(MabrurNelpDanSms))
