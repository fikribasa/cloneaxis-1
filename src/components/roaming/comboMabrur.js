...
import { getPackageComboMabrur } from '../../publics/redux/actions/package/roaming'

class ComboMabrur extends Component {
    componentDidMount = async () => {
        if (this.props.combomabrur === undefined) await this.props.dispatch(getPackageComboMabrur())
    }

    render() {
        let { combomabrur } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(ComboMabrur))
