...
import { getPackageVideo } from '../../publics/redux/actions/package/newBoostr'

class BoostrVideo extends Component {
    componentDidMount = async () => {
        if (this.props.boostrVideo === undefined) await this.props.dispatch(getPackageVideo())
    }

    render() {
        let { boostrVideo } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(BoostrVideo))
