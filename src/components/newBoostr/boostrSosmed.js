...
import { getPackageSosmed } from '../../publics/redux/actions/package/newBoostr'

class BoostrSosmed extends Component {
    componentDidMount = async () => {
        if (this.props.boostrSosmed === undefined) await this.props.dispatch(getPackageSosmed())
    }

    render() {
        let { boostrSosmed } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(BoostrSosmed))
