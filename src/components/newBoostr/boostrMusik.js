...
import { getPackageMusik } from '../../publics/redux/actions/package/newBoostr'

class BoostrMusik extends Component {
    componentDidMount = async () => {
        if (this.props.boostrMusik === undefined) await this.props.dispatch(getPackageMusik())
    }

    render() {
        let { boostrMusik } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(BoostrMusik))
