...
import { getPackageGame } from '../../publics/redux/actions/package/newBoostr'

class BoostrGame extends Component {
    componentDidMount = async () => {
        if (this.props.boostrGame === undefined) await this.props.dispatch(getPackageGame())
    }

    render() {
        let { boostrGame } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(BoostrGame))
