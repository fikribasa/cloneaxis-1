...
import { getPackageSunset } from '../../publics/redux/actions/package/newBoostr'

class BoostrSunset extends Component {
    componentDidMount = async () => {
        if (this.props.boostrSunset === undefined) await this.props.dispatch(getPackageSunset())
    }

    render() {
        let { boostrSunset } = this.props
        return (
            <View style={{ flex: 1 }}>
...
export default withNavigation(connect(mapStateToProps)(BoostrSunset))
