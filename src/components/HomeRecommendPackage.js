...
class HomeRecommendPackage extends Component {
    componentDidMount = () => {
        this.props.dispatch(getPackageCategoryRecommended())
    }
                            ...
                            _.take(this.props.recommended, 2).map(recommend => (
                                <TouchableOpacity style={ styles.card } onPress={() => this.props.navigation.navigate('PackageDetails', { package: recommend })} key={recommend.id}>
                                    ...
                                    <View style={{ flexDirection: 'row', marginTop: 5 }}>
                                        ...
                                        <Text style={{ fontSize: 17, fontWeight: 'bold', color: 'white' }}>{ recommend.discprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") }</Text>
                                    </View>
                                    {
                                        (recommend.discount > 0) ? <Text style={{ textDecorationLine: 'line-through', fontSize: 12, color: 'white' }}>Rp. {recommend.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") }</Text> : null
                                    }
                                    <Text style={{ fontSize: 9, marginTop: 8, color: 'white' }}>Masa aktif { recommend.duration } hari</Text>
                                </TouchableOpacity>
                            ))
                        : null
                    }
