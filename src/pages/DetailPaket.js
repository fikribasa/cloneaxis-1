...
import { View, Text, Image, TouchableOpacity, StyleSheet, ScrollView, ToastAndroid } from 'react-native'
...
class DetailPaket extends Component {
    state = { 
     ...
     componentDidMount = async() => {
         await this.setState({ products: this.props.navigation.state.params.transactions.rows})
     }
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    {
                        this.state.products.map(product => (
                            <View style={{ height: 150, elevation: 7, backgroundColor: 'white', marginHorizontal: 10, marginBottom: 20 }} key={product.Product.id}>
                                <View style={{ padding: 10 }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <Text style={{ fontSize: 25, color: '#8D7693', fontWeight: 'bold' }}>Paket</Text>
                                        <TouchableOpacity style={{ flexDirection: 'row' }} onPress={() => this.handleDelete(product.id)}>
                                            <Image source={require('../assets/icon/icon_delete.webp')} style={{ height: 20, width: 20, marginRight: 5 }} />
                                            <Text style={{ fontWeight: 'bold', color: '#EF018B' }} >BERHENTI</Text>
                                        </TouchableOpacity>
                                    </View>
                                    <Text style={{ fontWeight: 'bold', color: '#4C475B' }}>{product.Product.name}</Text>
                                    <Text style={{ color: '#DAD8DB' }}>Berlaku sampai 21 Agt 2019</Text>
                                    <View style={{ height: 1, backgroundColor: '#DAD8DB', marginTop: 8, marginBottom: 5 }} />
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 5 }}>
                                        ...
                                        <Text style={{ fontSize: 15, color: '#646466', fontWeight: 'bold' }}>{product.Product.bandwidth}MB</Text>
                                    </View>
